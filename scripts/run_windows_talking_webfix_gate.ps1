...
function Run-Scenario([string]$Label, [string]$BaselinePackage, [string]$JewelryPackage, [bool]$WithJewelry, [bool]$Interrupted, [bool]$Repeat) {
  ...
  if ($Interrupted) {
    ...
    Copy-Item -LiteralPath (Join-Path $Harness 'payload\scripts\workbench-artifacts\skill_artifact_adapter.py') -Destination $partialTarget -Force
    Set-Content -LiteralPath (Join-Path $workspace '系统文件_无需打开\talking-webfix.partial') -Value 'simulated interruption' -Encoding UTF8
  }
  & (Join-Path $Harness 'apply-test-overlay.ps1') -Workspace $workspace -Evidence (Join-Path $EvidenceRoot "$Label-overlay")
  Assert-Preservation $sentinels
  Run-Browser $workspace $skills $Label
  if ($Repeat) {
    Stop-ManagedServices $workspace
    & (Join-Path $Harness 'apply-test-overlay.ps1') -Workspace $workspace -Evidence (Join-Path $EvidenceRoot "$Label-overlay-repeat")
    Assert-Preservation $sentinels
