...
class WindowsReleaseGateTests(unittest.TestCase):
    ...
    def test_talking_head_contract_uses_installed_runtime_and_zero_cost_synthetic_input(self) -> None:
        ...
        self.assertIn('"--form-string", "native_run_strategy=economy"', gate_script)
        self.assertIn('"--form-string", "native_workflow_variant=production"', gate_script)
        self.assertIn('"--form-string", "quality_mode=clear"', gate_script)
        self.assertIn('"--form-string", "target_ratio=9:16"', gate_script)
        self.assertIn("& curl.exe @curlArgs", gate_script)
        self.assertNotIn("[System.Net.Http.MultipartFormDataContent]", gate_script)
