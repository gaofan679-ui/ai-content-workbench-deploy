...
export function artifactRoot(env = process.env) {
  return (
    String(env.WORKBENCH_ARTIFACT_ROOT || "").trim() ||
    String(env.AI_WORKBENCH_HOME || "").trim() ||
    join(userHome(env), "AIContentWorkbench")
  );
}
...
export function pythonUtf8Env(env = process.env) {
  return {
    ...env,
    PYTHONUTF8: "1",
