...
$Payload = Join-Path $Harness 'payload'
...
foreach ($file in $Manifest.files) {
  ...
  if ($sourceHash -ne [string]$file.sha256) { throw "Candidate file hash mismatch: $source" }
...
& tar.exe -xzf $distArchive -C $WebRoot
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath (Join-Path $dist 'server\index.js'))) { throw 'Candidate dist extraction failed.' }
