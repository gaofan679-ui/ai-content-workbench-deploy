...
客户交付体验固定为：一个 GitHub 入口 + 一个客户专属票据链接 + 一段短指令。客户不接收本地 ZIP，也不需要下载或上传票据文件。Windows 直接 HTTPS 读取失败时，由安装器自动调用本机 Edge/Chrome 回退，客户只在写入前确认一次。

公开仓库不得包含 ZIP、客户票据、签名 URL、API Key、Cookie、客户资料或本地绝对路径。
